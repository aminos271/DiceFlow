...
class UnavailableNarrationDynamicWorldLLM:
    def generate_dynamic_world(self, world, action, validation, state):
...
class DynamicWorldTest(unittest.TestCase):
    ...
    def test_unavailable_narration_llm_falls_back_to_builtin_world_patch(self) -> None:
        script = load_script("tomb_entrance")
        script["world"] = {
            "premise": "古墓入口及其后方未知墓道",
            "tone": "潮湿、黑暗、古代机关、低魔",
        ...
        game = Game(script=script, use_llm=False)
        game.llm = UnavailableNarrationDynamicWorldLLM()
        game.state.apply_changes(
            {
                "entities": {
                    "guard_1": {"hp_delta": -6},
        ...
        self.assertEqual(record.validation["reason"], "dynamic_world")
        self.assertEqual(game.state.scene["name"], "未知通道")
        self.assertTrue(game.state.flags["dynamic_world_used"])
